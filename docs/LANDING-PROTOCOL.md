...
- **Tiers 1 and 2 are for CLAUDE-AUTHORED work only.** Cursor-lane work never uses them: it reaches `main` ONLY as a reviewed pull-request merge through the lane in `docs/CURSOR-LANE.md` (CI on every PR + Grok diff critique + a Claude gate merge). Honest limit: no branch protection exists on this repo, so nothing mechanically stops a misconfigured agent from pushing — the gate checks the PR Files-changed list by hand and the Owner scopes Cursor's GitHub access; mechanization is open items 2 and 5.
