...
- **LEAN SCRIBE deferral (v4.1.16) — the CONTENT check moves, it is never dropped.** A gate SCRIBE landing a `gate_ratification` record may defer the check above to the NEXT run's self-brief, which reads the mirror and JOURNALS what it found as the compensating control. Conditions, all required: the landing NAMES the exact strings and files the next run must check; the work stays UNVERIFIED until that read is journaled, and is described that way rather than as verified; and when several runs are batch-gated, EACH deferral is named separately and the next run reads ALL of them — one read does not discharge N deferrals. The gate may always order the check at landing instead, and should whenever the landing carries a version bump or a rule change. The cost is stated, not waived: the window between landing and verification is now one RUN wide instead of one turn wide, so a broken sync surfaces later, and if the next run never happens it never surfaces at all.
- **Non-mirrored paths are verified in the repo, not on Drive.** The sync covers `docs/**` only, so `AGENTS.md`, `tools/**` and `.github/**` never appear in the mirror and a Drive CONTENT check cannot cover them. Verify those by reading the standing clone or `git show` at the landed SHA, and say which surface you read — claiming Drive verification for a file Drive never receives is a false green with a plausible shape.
...
- **Tiers 1 and 2 are for CLAUDE-AUTHORED work only.** Cursor-lane work never uses them: it reaches `main` ONLY as a reviewed pull-request merge through the lane in `docs/CURSOR-LANE.md` (CI on every PR + Grok diff critique + a Claude gate merge). Corrected 2026-08-19 (v4.1.16), because this bullet contradicted `docs/CURSOR-LANE.md`: branch protection DOES exist — ruleset **protect-main-cursor-lane** is Active, `main` requires pull requests, with bypass for Repository admin + the Claude app so Tier 1/2 landings still work (the gate merge 5ce249f is the proof). What is still NOT mechanical: the SCOPE of a PR's Files-changed list, which the gate checks by hand, and the Owner's scoping of Cursor's GitHub access; mechanization of both remains open items 2 and 5.
- **A landing that touched `docs/**` or `tools/**` carries regenerated briefings.** Run `python3 tools/gen_brief.py` before the commit (after `docs/PATCH-NOTES-CURRENT.md` and `docs/LATEST-HANDOFF.md` are final, because the generator reads them) and `python3 tools/gen_brief.py --check` as part of the pre-land step: `--check` exits 1 when any generated file is missing or would change, which is the only detector for a DELETED pack — the validator's staleness check skips cleanly on an absent pack and will not notice.
- **Every landing updates `docs/LATEST-HANDOFF.md` in the SAME commit**, overwriting it with this run's HANDOFF block (`docs/HANDOFF-FORMAT.md`). It is the Fable gate's inbox under efficiency mode (`docs/EFFICIENCY-MODE.md`): a landing that leaves it stale hands the gate the previous run's facts.
