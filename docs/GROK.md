...
## GROK CONTEXT RULE — every prompt file starts with the current snapshot (v4.1.16)

**Every Grok prompt file — critique pass, drafting, research, large-text summarization — BEGINS
with the current contents of `docs/GROK-CONTEXT.txt`, then the pass-specific ask below it.** That
file is GENERATED by `tools/gen_brief.py` from the canonical documents and regenerated on every
run that touches `docs/**` or `tools/**`, so Grok is briefed on the current system by
construction rather than by whoever remembered to paste a paragraph. Do not hand-edit it, do not
paraphrase it, and do not substitute an older copy: the whole point is that the snapshot in the
prompt matches the repo the critique is about.

It is built to the CLI's constraints, and the generator ENFORCES them rather than trusting the
author: pure ASCII, zero double quotes, zero apostrophes, no line beginning with a dash, and a
1500-character ceiling. Those are the same rules the prompt-file mechanics require, so prepending
the snapshot cannot break a prompt file that was otherwise valid.

**What it is not.** It is a HEADER, not context Grok can act on as authority, and not a substitute
for the Drive links or the pasted diff hunks — the change under review still goes in the prompt.
The snapshot is deliberately short, so the last-land line is TRUNCATED when the history row is
long, and the truncation is MARKED `[TRUNCATED]` rather than hidden. A pass that turns on a
detail the snapshot could not carry needs that detail pasted in explicitly; treating a
1500-character header as a full briefing is the failure mode to watch for. `docs/BRIEF-PACK.md`
is on the Drive mirror for exactly this reason and the snapshot points at it.

**The version-lag trap, and the line that defuses it.** On PRE-LAND critique the snapshot names
the version being BUILT while the Drive mirror still shows the PREVIOUS one, so a reviewer told
`v4.1.16` who then reads a `v4.1.15` mirror can reasonably conclude the change is missing, or
critique the old text as if it were the new. The prompt must therefore state plainly that the
change is pre-land, that the mirror lags by one version, and that the summary plus pasted hunks
are the artifact under review — a sentence this system has to write because the generated header
cannot know whether it is being used before or after a landing. Post-land, the same header is
accurate and no such line is needed. Grok flagging this mismatch is a correct finding, not noise.
Say the same about the pointer: pre-land, `docs/BRIEF-PACK.md` is NOT yet on the mirror either, so
a prompt that leans on it is pointing at a file the reviewer cannot open. Pre-land, the pasted
summary and diff hunks ARE the artifact — say so, and paste anything a bullet must turn on.

**The ASCII transform is a HAZARD on quoted rule text, and the rule is: say what you stripped.**
The prompt-file constraints (no double quotes, no apostrophes, no dash-led lines) apply to the
WHOLE file, so text pasted under review passes through the same transform: `Owner's` becomes
`Owners`, a quoted sentence loses its quotation marks, and a dash-led bullet becomes a star. On
wording-sensitive critique that is a real risk of reviewing text that was never written. Two
rules follow. (1) `docs/GROK-CONTEXT.txt` deliberately contains NO verbatim rule quotations — it
is a structural header, which is why the transform is safe there. (2) When a pass turns on EXACT
wording, say in the prompt that quotes and apostrophes were stripped for CLI compatibility and
that the stripping is not the change under review; otherwise Grok correctly critiques an artifact
of the transport. Where exact bytes matter more than the CLI's convenience, use the browser
fallback and record the flip.

**When the prompt runs long.** Prepending the snapshot costs about 1.5k characters, so prompts
that were already near the guideline will exceed it. That is allowed under the existing rule and
under the same compensating control: echo `PROMPT_CHARS` for the file and record it in the run,
so an over-length prompt is a measured decision rather than an accident.
...
**Before any block below, prepend the current `docs/GROK-CONTEXT.txt` verbatim** (GROK CONTEXT
RULE, above). The blocks are the pass-specific ask; the snapshot is the standing header.
